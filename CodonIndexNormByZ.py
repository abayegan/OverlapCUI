...
#A. Bayegan
#This program uses forward and backward partition functions obtained from Overcode software to 
...
import os,sys,math
from Overcode import computeZB,computeZF,computeZF_GC,computeZB_GC,createListOfCompatible5mers,NUCL,bfs,parseConstraintFile
...
#compute probability of codon w in the set of all sequences that translate 'peptides' in the corresponding reading frames
	...
	if gclow != -1: #if gc-content is defined
				...
				for x1 in range(0,3*k+3):
					for x2 in range(0,3*(n-k)+3):	
						...
						if( gclow <= x1 + x2 + GCcont(s[2]) <= gcup) : 
							...
							if '+0' in rf : freq[s[0:3]]+= (ZF[(k,x1,s[0],s[1])]/ZFval) * ZB[(k+1,x2,s[3],s[4])]
							if '+1' in rf : freq[s[1:4]]+= (ZF[(k,x1,s[0],s[1])]/ZFval) * ZB[(k+1,x2,s[3],s[4])]
							if '+2' in rf : freq[s[2:5]] += (ZF[(k,x1,s[0],s[1])]/ZFval) * ZB[(k+1,x2,s[3],s[4])]
							if '-0' in rf : freq[s[0:3][::-1]]+= (ZF[(k,x1,s[0],s[1])]/ZFval) * ZB[(k+1,x2,s[3],s[4])]
							if '-1' in rf : freq[s[1:4][::-1]]+= (ZF[(k,x1,s[0],s[1])]/ZFval) * ZB[(k+1,x2,s[3],s[4])]
							if '-2' in rf : freq[s[2:5][::-1]] += (ZF[(k,x1,s[0],s[1])]/ZFval) * ZB[(k+1,x2,s[3],s[4])]
		...
		for k in range(n):
			...
			L = createListOfCompatible5mers(hexa,filename,threshold)
			for s in L:
				if '+0' in rf : freq[s[0:3]]+= (ZF[(k,s[0],s[1])]/ZFval) * ZB[(k+1,s[3],s[4])]
				if '+1' in rf : freq[s[1:4]]+= (ZF[(k,s[0],s[1])]/ZFval) * ZB[(k+1,s[3],s[4])]
				if '+2' in rf : freq[s[2:5]] += (ZF[(k,s[0],s[1])]/ZFval) * ZB[(k+1,s[3],s[4])]
				if '-0' in rf : freq[s[0:3][::-1]]+= (ZF[(k,s[0],s[1]/ZFval)]) * ZB[(k+1,s[3],s[4])]
				if '-1' in rf : freq[s[1:4][::-1]]+= (ZF[(k,s[0],s[1]/ZFval)]) * ZB[(k+1,s[3],s[4])]
				if '-2' in rf : freq[s[2:5][::-1]] += (ZF[(k,s[0],s[1]/ZFval)]) * ZB[(k+1,s[3],s[4])]
...
def printusage(name):
	print "%s -f fastaFile -r readingFrames -c constraint -m BlosumMatrix -t threshold -gc GC_content" % name
	sys.exit(1)
...
if __name__=='__main__':
	...
	for i in range(len(args)):
		if args[i]=='-f':
			...
			print "fasta file must be specified!\n"
			printusage(sys.argv[0])
